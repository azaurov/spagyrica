...
export default function RemedyScreen() {
  ...
  const filteredProps = useMemo(() => {
    ...
    return q ? ALL_PROPERTIES.filter(p => p.toLowerCase().includes(q)) : ALL_PROPERTIES;
  }, [query]);

  const matchingPlants = useMemo(() => {
    if (selected.length === 0) return [];
    return PLANTS.filter(plant =>
      selected.every(prop => plant.properties.includes(prop))
    );
  }, [selected]);
  ...
  return (
    <View style={styles.root}>
      <ScrollView style={styles.scroll} contentContainerStyle={styles.container} keyboardShouldPersistTaps="handled">
        ...
        <Text style={styles.sub}>Select properties or symptoms to find matching herbs</Text>
        ...
        <View style={styles.chipsWrap}>
          {filteredProps.map(prop => {
            const isOn = selected.includes(prop);
            return (
              <TouchableOpacity
                key={prop}
                ...
                activeOpacity={0.7}
              >
                <Text style={[styles.chipText, isOn && styles.chipTextActive]}>{prop}</Text>
              </TouchableOpacity>
            );
          })}
          {filteredProps.length === 0 && (
            <Text style={styles.noResults}>No properties match "{query}"</Text>
          )}
        </View>
          ...
          <>
            ...
            <Text style={styles.resultsHeading}>
              {matchingPlants.length === 0
                ? 'No herbs match all selected properties'
                ...
                <View style={styles.resultInfo}>
                  ...
                  <View style={styles.propTags}>
                    {plant.properties.map(prop => (
                      <View
                        key={prop}
                        style={[styles.propTag, selected.includes(prop) && styles.propTagMatch]}
                      >
                        <Text style={[styles.propTagText, selected.includes(prop) && styles.propTagTextMatch]}>
                          {prop}
                        </Text>
                      </View>
                    ))}
...
const styles = StyleSheet.create({
  ...
  chipActive: { borderColor: C.gold, backgroundColor: 'rgba(201,168,76,0.15)' },
  ...
  propTags: { flexDirection: 'row', flexWrap: 'wrap', gap: 4, marginTop: 8 },
  propTag: {
  ...
  propTagMatch: { backgroundColor: 'rgba(201,168,76,0.18)', borderWidth: 1, borderColor: C.border },
  propTagText: { fontSize: 11, color: C.muted },
  propTagTextMatch: { color: C.gold, fontWeight: '600' },
